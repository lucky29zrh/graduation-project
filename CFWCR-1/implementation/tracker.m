...
for frame = 1:num_frames
    ...
    if frame > 1
        ...
        while iter <= params.refinement_iterations && any(old_pos ~= pos)
            ...
            xt_proj = cellfun(@(feat_map, cos_window) bsxfun(@times, feat_map, cos_window),...
                xt_proj, cos_window, 'uniformoutput', false);
            xtf_proj = cellfun(@cfft2, xt_proj, 'uniformoutput', false);

            xtf_proj = interpolate_dft(xtf_proj, interp1_fs, interp2_fs);
            scores_fs_feat = cellfun(@(hf, xf, pad_sz) padarray(sum(bsxfun(@times, hf, xf),...
                3), pad_sz),hf_full, xtf_proj, pad_sz, 'uniformoutput', false);
            scores_fs = permute(sum(cell2mat(scores_fs_feat), 3), [1 2 4 3]);
            [trans_row, trans_col, scale_ind] = optimize_scores(scores_fs,...
                params.newton_iterations);
            translation_vec = [trans_row, trans_col] .* (img_support_sz./output_sz)...
                * currentScaleFactor * scaleFactors(scale_ind);
            ...
            if nScales > 0 && params.use_scale_filter
                scale_change_factor = scale_filter_track(im, pos, base_target_sz,...
                    currentScaleFactor, scale_filter, params);
            end 
