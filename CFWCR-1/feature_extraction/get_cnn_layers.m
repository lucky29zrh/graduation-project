...
for k = 1:length(fparams.output_layer)    
    if fparams.downsample_factor(k) == 1
        feature_map{k} = cnn_feat(fparams.output_layer(k) + 1).x(fparams.start_ind(k,1):fparams.end_ind(k,1),...
            fparams.start_ind(k,2):fparams.end_ind(k,2), :, :);
    else
        feature_map{k} = average_feature_region(cnn_feat(fparams.output_layer(k) + 1).x(fparams.start_ind(k,1):fparams.end_ind(k,1),...
            fparams.start_ind(k,2):fparams.end_ind(k,2), :, :), fparams.downsample_factor(k));
    end
end
